...
with tab_host:
    ...
    st.caption(
        "Start your camera and microphone, hold the meeting normally, and let MeetFlow "
        "capture the conversation for automatic AI transcription, summary, action items, email, and MoM."
    )
    ...
    if host_mode == "Create a new meeting":
        host_title = st.text_input(
            "Meeting title",
            value=f"Live Meeting - {datetime.now().strftime('%Y-%m-%d')}",
            key="host_title",
        )
        host_name = st.text_input("Host name", placeholder="Your name", key="host_name")
        host_participants_raw = st.text_input(
            "Participants (comma-separated)",
            placeholder="Alice, Rahul, Priya",
            key="host_participants",
        )
        host_agenda = st.text_area(
            "Agenda",
            placeholder="1. Project progress\n2. Budget\n3. Next steps",
            height=120,
            key="host_agenda",
        )
        ...
        if st.button("🚀 Create Meeting Room", type="primary", key="create_room"):
            ...
            st.session_state.pop("meeting_recording_path", None)
    ...
    if active_code:
        ...
        if room:
            ...
            if not room.get("started_at"):
                ...
                # -------------------- LIVE VIDEO + MIC --------------------
                st.markdown("### 🎥 Live Meeting Studio")
                st.caption(
                    "Allow camera and microphone access when Chrome asks. "
                    "The video preview is live and the incoming media is recorded for AI transcription."
                )

                if "meeting_recording_path" not in st.session_state:
                    st.session_state["meeting_recording_path"] = str(
                        Path(VIDEO_DIR) / f"{active_code}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.webm"
                    )
                recording_path = Path(st.session_state["meeting_recording_path"])

                def meeting_recorder_factory():
                    # Records the host camera + microphone stream.
                    # streamlit-webrtc's MediaRecorder is the same recording pattern
                    # used in its official recording example.
                ...
                webrtc_ctx = webrtc_streamer(
                    key=f"meetflow-room-{active_code}",
                    ...
                    in_recorder_factory=meeting_recorder_factory,
                )
                ...
                st.markdown(
                    ...
                    <div style="
                        margin:10px 0 16px;
                        padding:12px 15px;
                        border:1px solid rgba(6,214,245,.25);
                        border-radius:12px;
                        background:linear-gradient(135deg,rgba(7,33,78,.72),rgba(2,10,27,.82));
                        color:#b8c7d9;
                        font-size:13px;">
                        🎙️ <b style="color:#fff;">AI Listening Mode:</b>
                        MeetFlow records the meeting audio automatically while the camera preview is active.
                        You do not need to type live notes.
                    </div>
                    """,
                    unsafe_allow_html=True,
                )

                # Keep optional notes, but make them clearly optional.
                with st.expander("📝 Optional private notes", expanded=False):
                    optional_notes = st.text_area(
                        "Notes",
                        value=room.get("notes", ""),
                        height=120,
                        placeholder="Optional: add anything the microphone may not capture, such as whiteboard details.",
                        key=f"optional_notes_{active_code}",
                    )

                    if st.button("💾 Save Optional Notes", key="save_optional_notes"):
                        update_meeting_room(active_code, notes=optional_notes)
                        st.success("Optional notes saved.")

                # The WebRTC widget has its own STOP button. Ask the user to stop
                # the recording first so the MediaRecorder finishes writing the file.
                if webrtc_ctx.state.playing:
                    st.info("🔴 Meeting is live — use the STOP button inside the video panel when the conversation is finished.")
                elif recording_path.exists() and recording_path.stat().st_size > 0:
                    st.success("✅ Camera/microphone recording is ready for AI processing.")

                if not room.get("ended_at"):
                    end_disabled = not recording_path.exists() or recording_path.stat().st_size == 0

                    if st.button(
                        "🛑 End Meeting & Generate AI Minutes",
                        type="primary",
                        key="end_room",
                        ...
                        if not GOOGLE_API_KEY:
                            ...
                            try:
                                # Use the recorded meeting as the source of truth.
                                update_meeting_room(
                                    active_code,
                                    notes=optional_notes if "optional_notes" in locals() else room.get("notes", ""),
                                    ended_at=datetime.now().isoformat(timespec="seconds"),
                                )

                                with st.spinner("🎙️ Uploading meeting recording to Gemini..."):
                                    transcript_text = transcribe_audio(str(recording_path))

                                llm = get_llm()

                                with st.spinner("🤖 AI is preparing your meeting minutes..."):
                                    room_summary = summarize_meeting(
                                        llm,
                                        transcript_text,
                                        language_instruction,
                                    )
                                    room_actions = extract_action_items(
                                        llm,
                                        transcript_text,
                                        team_members,
                                        language_instruction,
                                    )
                                    room_email = draft_followup_email(
                                        llm,
                                        room_summary,
                                        room_actions,
                                        room["title"],
                                ...
                                # Add optional private notes to the saved transcript
                                # only when the host actually entered something.
                                notes_text = optional_notes.strip() if "optional_notes" in locals() else ""
                                if notes_text:
                                    transcript_for_history = (
                                        transcript_text
                                        + "\n\n[Host's optional private notes]\n"
                                        + notes_text
                                    )
                                else:
                                    transcript_for_history = transcript_text
                                ...
                                record = {
                                    ...
                                    "transcript": transcript_for_history,
                ...
                if host_result:
                    ...
                    st.text_area(
                        "Email",
                        value=host_result["email"],
                        height=220,
                        key="host_email_result",
                    )

                    try:
                        host_pdf = generate_mom_pdf(
                            host_result["title"],
                            host_result["date"],
                            host_result["summary"],
                            host_result["actions"],
    ...
    if host_mode == "Join an existing meeting":
        join_code = st.text_input(
            "Enter 6-character meeting code",
            max_chars=6,
            key="join_code",
        ).strip().upper()
    ...
    st.caption(
        "MeetFlow uses your browser camera/microphone for the host preview and records the meeting media "
        "for AI transcription. Full multi-person video conferencing still requires a signaling/TURN layer."
    )
