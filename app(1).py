...
LATIN_REGULAR = os.path.join(FONT_DIR, "DejaVuSans.ttf")
LATIN_BOLD = os.path.join(FONT_DIR, "DejaVuSans-Bold.ttf")
UNICODE_FONT_AVAILABLE = (
    os.path.exists(FONT_REGULAR)
    and os.path.exists(FONT_BOLD)
    and os.path.exists(LATIN_REGULAR)
    and os.path.exists(LATIN_BOLD)
)
...
def _write_mixed_text(pdf: FPDF, text: str, line_height: float, bold: bool = False):
    """Write Hindi + English/Hinglish safely using the correct font for each script."""
    if not text:
        pdf.ln(line_height)
        return

    # Devanagari Unicode range. Everything else is rendered with DejaVu Sans.
    pattern = r"([\u0900-\u097F\uA8E0-\uA8FF]+)"
    ...
    for part in parts:
        if not part:
            continue
        is_devanagari = bool(re.fullmatch(r"[\u0900-\u097F\uA8E0-\uA8FF]+", part))
        if is_devanagari:
            pdf.set_font("NotoDev", "B" if bold else "", 11 if not bold else 10.5)
        else:
            pdf.set_font("DejaVu", "B" if bold else "", 11 if not bold else 10.5)
...
def generate_mom_pdf(meeting_title: str, meeting_date: str, summary: str, action_items: list) -> bytes:
    """Generate a proper multilingual Minutes of Meeting PDF.

    Noto Sans Devanagari is used only for Devanagari characters and DejaVu Sans
    for English/Hinglish. This avoids the missing-Latin-glyph problem in the
    Devanagari font while still supporting Hindi. HarfBuzz shaping is enabled
    for correct Devanagari conjuncts and vowel marks.
    """
    if not UNICODE_FONT_AVAILABLE:
        raise RuntimeError(
            "PDF fonts are missing. The repository must contain a fonts folder "
            "with NotoSansDevanagari-Regular.ttf, NotoSansDevanagari-Bold.ttf, "
    ...
    pdf.add_font("NotoDev", "", FONT_REGULAR)
    pdf.add_font("NotoDev", "B", FONT_BOLD)
    pdf.add_font("DejaVu", "", LATIN_REGULAR)
    pdf.add_font("DejaVu", "B", LATIN_BOLD)

    # Required by fpdf2 for proper Devanagari shaping.
    pdf.set_text_shaping(True)
    pdf.add_page()

    w = pdf.epw

    # Title — English, so use the Latin font directly.
    pdf.set_font("DejaVu", "B", 18)
    pdf.multi_cell(w, 10, "MINUTES OF MEETING", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(4)

    # Meeting information
    pdf.set_font("DejaVu", "B", 11)
    pdf.multi_cell(w, 7, "Meeting Title", new_x="LMARGIN", new_y="NEXT")
    _write_mixed_text(pdf, _sanitize_for_pdf(meeting_title), 6.5)
    _write_mixed_text(pdf, f"Date: {meeting_date}", 6.5)
    pdf.ln(4)

    # Summary heading
    pdf.set_font("DejaVu", "B", 14)
    pdf.multi_cell(w, 8, "Summary", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(1)
    ...
    for raw_line in clean_summary.splitlines():
        ...
        if line.startswith("* ") or line.startswith("- "):
            line = "• " + line[2:]
        _write_mixed_text(pdf, line, 6.5)

    pdf.ln(4)

    # Action items
    pdf.set_font("DejaVu", "B", 14)
    pdf.multi_cell(w, 8, "Action Items", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(1)

    if action_items:
        for index, item in enumerate(action_items, start=1):
            task = _sanitize_for_pdf(str(item.get("task", "")))
            owner = _sanitize_for_pdf(str(item.get("owner", "Unassigned")))
            deadline = _sanitize_for_pdf(str(item.get("deadline", "TBD")))
            status = _sanitize_for_pdf(str(item.get("status", "Pending")))

            _write_mixed_text(pdf, f"{index}. {task}", 6.5, bold=True)
            _write_mixed_text(
                pdf,
        ...
        _write_mixed_text(pdf, "No action items were identified.", 6.5)

    # Footer
    pdf.set_y(-15)
    pdf.set_font("DejaVu", "", 8)
    pdf.cell(0, 5, "Generated by AI Meeting Assistant", align="C")
