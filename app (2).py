...
def generate_mom_pdf(meeting_title: str, meeting_date: str, summary: str, action_items: list) -> bytes:
    ...
    pdf.multi_cell(w, 8, "Summary")
    pdf.set_font("Helvetica", "", 11)
    pdf.multi_cell(w, 6, _sanitize_for_pdf(summary))
    pdf.ln(4)

    pdf.set_font("Helvetica", "B", 13)
    pdf.multi_cell(w, 8, "Action Items")
    pdf.set_font("Helvetica", "", 11)
    if action_items:
        for i, item in enumerate(action_items, 1):
            line = (
        ...
        pdf.multi_cell(w, 6, "No action items identified.")
...
with tab_new:
    ...
    # downloading the email, then the PDF, then the Markdown MoM all just
    # work — no need to hit "Process Meeting" again between downloads.
    ...
    if result:
        ...
        mom_markdown = generate_mom_markdown(result["title"], result["mom_date_str"], result["summary"], result["action_items"])
        ...
        with col_md:
            st.download_button(
                ...
                data=mom_markdown,
                file_name=f"{result['title'].replace(' ', '_')}_MoM.md",
                key="dl_md_new",
            )
